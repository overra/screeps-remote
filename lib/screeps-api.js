...
const FIND_USER_URL = 'https://screeps.com/api/user/find?username='
const USER_ROOMS_URL = 'https://screeps.com/api/user/rooms?id='
const USER_INFO_URL = 'https://screeps.com/api/auth/me'
const OVERVIEW_URL = 'https://screeps.com/api/user/overview'
...
class Screeps extends EventEmitter {
  ...
  poll() {
    if (this.user && this.user.token) {
      ...
      // getOverview(this.username, this.user.token).then(overview => {
...
const getOverview = (username, token) => new Promise((resolve, reject) => {
  const options = {
    url: OVERVIEW_URL,
    json: true,
    headers: {
